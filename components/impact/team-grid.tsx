  ...
  return <div className="grid gap-4">{team.map((member) => {
    ...
    return <motion.article layout key={member.slug} id={member.slug} className="scroll-mt-28 overflow-hidden rounded-2xl border border-sky-300/25 bg-slate-50 text-slate-900 shadow-xl shadow-black/10">
      <button type="button" onClick={() => setOpen(expanded ? "" : member.slug)} aria-expanded={expanded} className="focus-ring flex w-full items-center gap-4 p-5 text-left md:px-7">
        <span className="relative grid size-14 shrink-0 place-items-center overflow-hidden rounded-xl bg-slate-900 text-sm font-bold text-white">{member.photo ? <Image src={member.photo} fill sizes="56px" className="object-cover" style={{ objectPosition: member.photoPosition }} alt="" /> : initials}</span>
      ...
      <AnimatePresence initial={false}>{expanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="grid gap-8 border-t border-slate-200 p-6 md:grid-cols-[.42fr_1fr] md:p-10">
        <div className="relative grid min-h-72 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 to-slate-700">{member.photo ? <Image src={member.photo} fill sizes="(max-width:768px) 100vw, 35vw" className="object-cover" style={{ objectPosition: member.photoPosition }} alt={`Portrait of ${member.name}`} /> : <div className="text-center"><span className="display text-6xl text-white">{initials}</span><p className="mt-3 text-xs uppercase tracking-[.2em] text-sky-300">Impact500 Research</p></div>}</div>
